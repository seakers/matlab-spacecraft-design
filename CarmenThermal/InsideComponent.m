...
for i=1:len
    ...
    if strcmp('Inside',complist(i).LocationReq)
        ...
        if complist(i).Thermal(2)<Thigh
            ...
            hotfun=@(x,a,b,c,d,e,f,g)a*b*(x-c-273)/g-0.3*d*e*b*((f+273)^4-x^4);
            a=MLI.conductivity;
            b=area;
        ...
        %    Qrad=0.3*sigma*epsilon*area*(x^4-(Tlow+273)^4);
            coldfun=@(x,a,b,c,d,e,f,g)a*b*(c+273-x)/g-0.3*d*e*b*(x^4-(f+273)^4);
            a=MLI.conductivity;
            b=area;
