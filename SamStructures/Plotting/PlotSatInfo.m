function PlotSatInfo(payload,comms,eps,avionics,thermal,structures, propulsion,adcs,LV)
...
power = [sum(payload.power),comms.power,avionics.AvgPwr,propulsion.power];
power_subsystemStrings = {'Payload','Comms','Avionics','Propulsion'};

% Create the vectors containing the cost info
cost = [avionics.Cost/1000, comms.cost, eps.cost, thermal.cost,propulsion.cost,adcs.cost];
cost_subsystemStrings = {'Avionics','Comms','EPS','Thermal','Propulsion','ADCS'};
